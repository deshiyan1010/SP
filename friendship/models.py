...
class FriendshipRequest(models.Model):
    ...
    def accept(self):
        ...
        bust_cache("requests", self.to_user.id)
        bust_cache("sent_requests", self.from_user.id)
        # Bust reverse requests cache - reverse request might be deleted
        bust_cache("requests", self.from_user.id)
        bust_cache("sent_requests", self.to_user.id)
        # Bust friends cache - new friends added
        bust_cache("friends", self.to_user.id)
        bust_cache("friends", self.from_user.id)
...
class FriendshipManager(models.Manager):
    ...
    def requests(self, user):
        ...
        key = cache_key("requests", user.id)
    ...
    def sent_requests(self, user):
        ...
        key = cache_key("sent_requests", user.id)
        requests = cache.get(key)
    ...
    def unread_requests(self, user):
        ...
        key = cache_key("unread_requests", user.id)
    ...
    def unread_request_count(self, user):
        ...
        key = cache_key("unread_request_count", user.id)
        count = cache.get(key)
    ...
    def read_requests(self, user):
        ...
        key = cache_key("read_requests", user.id)
    ...
    def rejected_requests(self, user):
        ...
        key = cache_key("rejected_requests", user.id)
    ...
    def unrejected_requests(self, user):
        ...
        key = cache_key("unrejected_requests", user.id)
    ...
    def unrejected_request_count(self, user):
        ...
        key = cache_key("unrejected_request_count", user.id)
        count = cache.get(key)
    ...
    def add_friend(self, from_user, to_user, message=None):
        ...
        bust_cache("requests", to_user.id)
        bust_cache("sent_requests", from_user.id)
    ...
    def remove_friend(self, from_user, to_user):
        ...
        try:
            ...
            if qs:
                ...
                qs.delete()
                bust_cache("friends", to_user.id)
                bust_cache("friends", from_user.id)
                return True
            else:
                return False
    ...
    def are_friends(self, user1, user2):
        ...
        friends1 = cache.get(cache_key("friends", user1.id))
        friends2 = cache.get(cache_key("friends", user2.id))
...
class FollowingManager(models.Manager):
    ...
    def following(self, user):
        ...
        key = cache_key("following", user.id)
    ...
    def add_follower(self, follower, followee):
        ...
        bust_cache("followers", followee.id)
        bust_cache("following", follower.id)
    ...
    def follows(self, follower, followee):
        ...
        followers = cache.get(cache_key("following", follower.id))
        following = cache.get(cache_key("followers", followee.id))
...
class BlockManager(models.Manager):
    ...
    def blocking(self, user):
        ...
        key = cache_key("blocking", user.id)
    ...
    def add_block(self, blocker, blocked):
        ...
        bust_cache("blocked", blocked.id)
        bust_cache("blocking", blocker.id)
    ...
    def is_blocked(self, user1, user2):
        ...
        block1 = cache.get(cache_key("blocks", user1.id))
        block2 = cache.get(cache_key("blocks", user2.id))
